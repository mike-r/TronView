...
# Dynon SV-ADSB-472 serial input source
# Based on GDL90 protocol decoder from stratx_wifi.py by Topher 2019
# 2/23/2025 Zap
...
class serial_skyview_adsb(Input):
    def __init__(self):
        self.name = "skyview_adsb"
        self.version = 1.0
    ...
    def initInput(self, num, dataship: Dataship):
        ...
        if(self.PlayFile!=None and self.PlayFile!=False):
            ...
            if self.PlayFile==True:
                defaultTo = "skyview_adsb_data1.dat"
            ...
            # For now we will just assume there is one serial interface and it is used for ADSB
    ...
    def getNextChunck(self, dataship: Dataship):
        ...
        try:
            # Look for Dynon ADSB flag sequence 0xC2 0x52 0xFF
            while True:
                b1 = self.ser.read(1)
                if not b1:  # Handle timeout/no data
                    if self.isPlaybackMode:
                        self.ser.seek(0)
                        print("Skyview ADSB file reset")
                    return dataship
                ...
                b2 = self.ser.read(1)
                if not b2 or b2[0] != 0x52:
                    continue
                    
                b3 = self.ser.read(1)
                if not b3 or b3[0] != 0xFF:
                    continue
                    
                break  # Found header sequence

            # Read the rest of the frame (32 bytes)
            dynon_special_1 = self.ser.read(1)  # TODO Figure out what 0x0B is
            dynon_special_2 = self.ser.read(1)  # TODO Figure out what 0x1B is
            dynon_special_3 = self.ser.read(1)  # should be 0x14 "Traffic Report"
            data = bytearray(b"~")              # insert GDL-90 flag byte
            if self.ser.read(1) == 0: data.extend(0x14)  # Traffic Report message type
            data.extend(self.ser.read(29))      # standard GDL90 frame internals
            data.extend(b"~")                   # insert GDL-90 flag byte
            if len(data) != 32:
                return dataship

            dynon_crc = self.ser.read(2)  # TODO Validate checksum
            dynon_frame_flag = self.ser.read(3)  # 0xC2 0x52 0xFF
               
        except serial.serialutil.SerialException as e:
            print(e)
            print("Skyvie ADSB serial exception")
            traceback.print_exc()
    ...
    def readMessage(self, dataship: Dataship):
        ...
        count = msg.count(0xFF)
        print("-----------------------------------------------\nNEW Chunk len:"+str(len(msg))+" seperator count:"+str(count))
        if(dataship.debug_mode>2):
            if len(msg) >= 4:
                print("skview_adsb: "+str(msg[1])+" "+str(msg[2])+" "+str(msg[3])+" "+str(len(msg))+" "+str(msg))
