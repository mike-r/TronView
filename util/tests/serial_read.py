        ...
        if dataType.decode() == "1":        # ADHARS Data Message
            ...
            if len(msg) == 91:
                ...
                if (CRLF[0]) == 13 or (CRLF[0]) == "\r":
                    ...
                    calcChecksumHex = "0x{:02x}".format(calcChecksum)
                    print_xy(5, 20, bcolors.OKGREEN + "NAV, AP, Misc (2)" + bcolors.ENDC)
                    print_xy(6, 20, "DataType:   %s" % (dataType.decode()))
                    print_xy(7, 20, "Ver:        %s" % (DataVer.decode()))
                    print_xy(8, 20, "SysTime:    %s" % (SysTime.decode()))
                    print_xy(9, 20, "Heading Bug: %s" % (HBug.decode()))
                    print_xy(10, 20, "Alt Bug:      %s" % (AltBug.decode()))
                    print_xy(11, 20, "AS Bug:       %s" % (AirBug.decode()))
                    print_xy(12, 20, "VS Bug:       %s" % (VSBug.decode()))
                    print_xy(13, 20, "Course:       %s" % (Course.decode()))
                    print_xy(14, 20, "CDI Type:     %s" % (CDISrcType.decode()))
                    print_xy(15, 20, "CDI Port:     %s" % (CDISourePort.decode()))
                    print_xy(16, 20, "CDI Scale:    %s" % (CDIScale.decode()))
                    print_xy(17, 20, "CDI Deflect   %s" % (CDIDeflection.decode()))
                    print_xy(18, 20, "VertSpd:      %s" % (GS.decode()))
                    print_xy(19, 20, "AP Engaged:   %s" % (APEng.decode()))
                    print_xy(20, 20, "AP Roll Mode: %s" % (APRollMode.decode()))
                    print_xy(21, 20, "AP Pitch Mode:%s" % (APPitch.decode()))
                    print_xy(22, 20, "Not-1:        %s" % (Not1.decode()))
                    print_xy(23, 20, "Not-2:        %s" % (Not2.decode()))
                    print_xy(24, 20, "Squawk Code:  %s" % (TransponderCode.decode()))
                    print_xy(25, 20, "ChkSum:     0x%s   int:%d " % (Checksum.decode(), intCheckSum))
                    print_xy(26, 20, "CalChkSum:    %s   int:%d " % (calcChecksumHex, calcChecksum))
                    nextByte = ser.read(1)
                    print_xy(27, 20, "endbyte:    %s " % (repr(CRLF[0])))
            ...
            if len(msg) == 223:
                ...
                if (CRLF[0]) == 13 or (CRLF[0]) == "\r":
                    ...
                    calcChecksumHex = "0x{:02x}".format(calcChecksum)
                    print_xy(5, 40, bcolors.OKGREEN + "Engine (3)" + bcolors.ENDC)
                    print_xy(6, 40,  "DataType:     %s" % (dataType.decode()))
                    print_xy(7, 40,  "Ver:          %s" % (DataVer.decode()))
                    print_xy(8, 40,  "SysTime:      %s" % (SysTime.decode()))
                    print_xy(9, 40,  "Oil Pressure: %s" % (OilPress.decode()))
                    print_xy(10, 40, "Oil Temp:     %s" % (OilTemp.decode()))
                    print_xy(11, 40, "RPM:          %s" % (RPM_L.decode()))
                    print_xy(12, 40, "MAP:          %s" % (MAP.decode()))
                    print_xy(13, 40, "Fuel Flow:    %s" % (FF1.decode()))
                    print_xy(14, 40, "Fuel Pressre: %s" % (FP.decode()))
                    print_xy(15, 40, "Fuel Left:    %s" % (FL_L.decode()))
                    print_xy(16, 40, "Fuel Right:   %s" % (FL_R.decode()))
                    print_xy(17, 40, "Fuel Rem      %s" % (Frem.decode()))
                    print_xy(18, 40, "Voltage-1:    %s" % (V1.decode()))
                    print_xy(19, 40, "Voltage-2:    %s" % (V2.decode()))
                    print_xy(20, 40, "AMPs:         %s" % (AMPs.decode()))
                    print_xy(21, 40, "Hobbs:        %s" % (Hobbs.decode()))
                    print_xy(22, 40, "EGT-1:        %s" % (TC1.decode()))
                    print_xy(23, 40, "CHT-1:        %s" % (TC8.decode()))
                    print_xy(24, 40, "Eng Power:    %s" % (Pwr.decode()))
                    print_xy(25, 40, "ChkSum:      0x%s   int:%d " % (Checksum.decode(), intCheckSum))
                    print_xy(26, 40, "CalChkSum:     %s   int:%d " % (calcChecksumHex, calcChecksum))
                    nextByte = ser.read(1)
                    print_xy(27, 40, "endbyte:       %s " % (repr(CRLF[0])))
        else:
            badmessageheaderCount += 1
            ser.flushInput()
            return
