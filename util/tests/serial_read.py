        ...
        if dataType.decode() == "1":        # ADHARS Data Message
            ...
            if len(msg) == 91:
                ...
                if (CRLF[0]) == 13 or (CRLF[0]) == "\r":
                    ...
                    calcChecksumHex = "0x{:02x}".format(calcChecksum)
                    print_xy(5, 25, bcolors.OKGREEN + "NAV, AP, Misc (2)" + bcolors.ENDC)
                    print_xy(6, 25, "DataType:   %s" % (dataType.decode()))
                    print_xy(7, 25, "Ver:        %s" % (DataVer.decode()))
                    print_xy(8, 25, "SysTime:    %s" % (SysTime.decode()))
                    print_xy(9, 25, "Heading Bug: %s" % (HBug.decode()))
                    print_xy(10, 25, "Alt Bug:      %s" % (AltBug.decode()))
                    print_xy(11, 25, "AS Bug:       %s" % (AirBug.decode()))
                    print_xy(12, 25, "VS Bug:       %s" % (VSBug.decode()))
                    print_xy(13, 25, "Course:       %s" % (Course.decode()))
                    print_xy(14, 25, "CDI Type:     %s" % (CDISrcType.decode()))
                    print_xy(15, 25, "CDI Port:     %s" % (CDISourePort.decode()))
                    print_xy(16, 25, "CDI Scale:    %s" % (CDIScale.decode()))
                    print_xy(17, 25, "CDI Deflect   %s" % (CDIDeflection.decode()))
                    print_xy(18, 25, "VertSpd:      %s" % (GS.decode()))
                    print_xy(19, 25, "AP Engaged:   %s" % (APEng.decode()))
                    print_xy(20, 25, "AP Roll Mode: %s" % (APRollMode.decode()))
                    print_xy(21, 25, "AP Pitch Mode:%s" % (APPitch.decode()))
                    print_xy(22, 25, "Not-1:        %s" % (Not1.decode()))
                    print_xy(23, 25, "Not-2:        %s" % (Not2.decode()))
                    print_xy(24, 25, "Squawk Code:  %s" % (TransponderCode.decode()))
                    print_xy(25, 25, "ChkSum:     0x%s   int:%d " % (Checksum.decode(), intCheckSum))
                    print_xy(26, 25, "CalChkSum:    %s   int:%d " % (calcChecksumHex, calcChecksum))
                    nextByte = ser.read(1)
                    print_xy(27, 25, "endbyte:    %s " % (repr(CRLF[0])))
            ...
            if len(msg) == 223:
                ...
                if (CRLF[0]) == 13 or (CRLF[0]) == "\r":
                    ...
                    calcChecksumHex = "0x{:02x}".format(calcChecksum)
                    print_xy(5, 50, bcolors.OKGREEN + "Engine (3)" + bcolors.ENDC)
                    print_xy(6, 50,  "DataType:     %s" % (dataType.decode()))
                    print_xy(7, 50,  "Ver:          %s" % (DataVer.decode()))
                    print_xy(8, 50,  "SysTime:      %s" % (SysTime.decode()))
                    print_xy(9, 50,  "Oil Pressure: %s" % (OilPress.decode()))
                    print_xy(10, 50, "Oil Temp:     %s" % (OilTemp.decode()))
                    print_xy(11, 50, "RPM:          %s" % (RPM_L.decode()))
                    print_xy(12, 50, "MAP:          %s" % (MAP.decode()))
                    print_xy(13, 50, "Fuel Flow:    %s" % (FF1.decode()))
                    print_xy(14, 50, "Fuel Pressre: %s" % (FP.decode()))
                    print_xy(15, 50, "Fuel Left:    %s" % (FL_L.decode()))
                    print_xy(16, 50, "Fuel Right:   %s" % (FL_R.decode()))
                    print_xy(17, 50, "Fuel Rem      %s" % (Frem.decode()))
                    print_xy(18, 50, "Voltage-1:    %s" % (V1.decode()))
                    print_xy(19, 50, "Voltage-2:    %s" % (V2.decode()))
                    print_xy(20, 50, "AMPs:         %s" % (AMPs.decode()))
                    print_xy(21, 50, "Hobbs:        %s" % (Hobbs.decode()))
                    print_xy(22, 50, "EGT-1:        %s" % (TC1.decode()))
                    print_xy(23, 50, "CHT-1:        %s" % (TC8.decode()))
                    print_xy(24, 50, "Eng Power:    %s" % (Pwr.decode()))
                    print_xy(25, 50, "ChkSum:      0x%s   int:%d " % (Checksum.decode(), intCheckSum))
                    print_xy(26, 50, "CalChkSum:     %s   int:%d " % (calcChecksumHex, calcChecksum))
                    nextByte = ser.read(1)
                    print_xy(27, 50, "endbyte:       %s " % (repr(CRLF[0])))
        else:
            badmessageheaderCount += 1
            ser.flushInput()
            return
