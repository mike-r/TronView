        ...
        if dataType.decode() == "1":        # ADHARS Data Message
            ...
            if len(msg) == 91:
                ...
                if (CRLF[0]) == 13 or (CRLF[0]) == "\r":
                    ...
                    print_xy(6, 30, "DataType:   %s" % (dataType.decode()))
                    print_xy(7, 30, "Ver:        %s" % (DataVer.decode()))
                    print_xy(8, 30, "SysTime:    %s" % (SysTime.decode()))
                    print_xy(9, 30, "Heading Bug: %s" % (HBug.decode()))
                    print_xy(10, 30, "Alt Bug:      %s" % (AltBug.decode()))
                    print_xy(11, 30, "AS Bug:       %s" % (AirBug.decode()))
                    print_xy(12, 30, "VS Bug:       %s" % (VSBug.decode()))
                    print_xy(13, 30, "Course:       %s" % (Course.decode()))
                    print_xy(14, 30, "CDI Type:     %s" % (CDISrcType.decode()))
                    print_xy(15, 30, "CDI Port:     %s" % (CDISourePort.decode()))
                    print_xy(16, 30, "CDI Scale:    %s" % (CDIScale.decode()))
                    print_xy(17, 30, "CDI Deflect   %s" % (CDIDeflection.decode()))
                    print_xy(18, 30, "VertSpd:      %s" % (GS.decode()))
                    print_xy(19, 30, "AP Engaged:   %s" % (APEng.decode()))
                    print_xy(20, 30, "AP Roll Mode: %s" % (APRollMode.decode()))
                    print_xy(21, 30, "AP Pitch Mode:%s" % (APPitch.decode()))
                    print_xy(22, 30, "Not-1:        %s" % (Not1.decode()))
                    print_xy(23, 30, "Not-2:        %s" % (Not2.decode()))
                    print_xy(24, 30, "Squawk Code:  %s" % (TransponderCode.decode()))
                    print_xy(25, 30, "ChkSum:     0x%s   int:%d " % (Checksum.decode(), intCheckSum))
                    print_xy(26, 30, "CalChkSum:    %s   int:%d " % (calcChecksumHex, calcChecksum))
                    nextByte = ser.read(1)
                    print_xy(27, 30, "endbyte:    %s " % (repr(CRLF[0])))
            ...
            if len(msg) == 223:
                ...
                if (CRLF[0]) == 13 or (CRLF[0]) == "\r":
                    ...
                    print_xy(6, 60,  "DataType:     %s" % (dataType.decode()))
                    print_xy(7, 60,  "Ver:          %s" % (DataVer.decode()))
                    print_xy(8, 60,  "SysTime:      %s" % (SysTime.decode()))
                    print_xy(9, 60,  "Oil Pressure: %s" % (OilPress.decode()))
                    print_xy(10, 60, "Oil Temp:     %s" % (OilTemp.decode()))
                    print_xy(11, 60, "RPM:          %s" % (RPM_L.decode()))
                    print_xy(12, 60, "MAP:          %s" % (MAP.decode()))
                    print_xy(13, 60, "Fuel Flow:    %s" % (FF1.decode()))
                    print_xy(14, 60, "Fuel Pressre: %s" % (FP.decode()))
                    print_xy(15, 60, "Fuel Left:    %s" % (FL_L.decode()))
                    print_xy(16, 60, "Fuel Right:   %s" % (FL_R.decode()))
                    print_xy(17, 60, "Fuel Rem      %s" % (Frem.decode()))
                    print_xy(18, 60, "Voltage-1:    %s" % (V1.decode()))
                    print_xy(19, 60, "Voltage-2:    %s" % (V2.decode()))
                    print_xy(20, 60, "AMPs:         %s" % (AMPs.decode()))
                    print_xy(21, 60, "Hobbs:        %s" % (Hobbs.decode()))
                    print_xy(22, 60, "EGT-1:        %s" % (TC1.decode()))
                    print_xy(23, 60, "CHT-1:        %s" % (TC8.decode()))
