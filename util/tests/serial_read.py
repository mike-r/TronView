        ...
        if dataType.decode() == "1":        # ADHARS Data Message
            ...
            # DataVer,SysTime = struct.unpack("c8s", msg)
                ...
                if (CRLF[0]) == 13 or (CRLF[0]) == "\r":
                    ...
                    print_xy(7, 0, "Pitch:      %s" % (pitch.decode()))
                    print_xy(8, 0, "Roll:       %s" % (roll.decode()))
                    print_xy(9, 0, "HeadMag:    %s" % (HeadingMAG.decode()))
                    print_xy(10, 0, "IAS:        %s" % (IAS.decode()))
                    print_xy(11, 0, "PresAlt:    %s" % (PresAlt.decode()))
                    print_xy(12, 0, "TurnRate:   %s" % (TurnRate.decode()))
                    print_xy(13, 0, "LatAccel:   %s" % (LatAccel.decode()))
                    print_xy(14, 0, "VertAccel:  %s" % (VertAccel.decode()))
                    print_xy(15, 0, "AOA:        %s" % (AOA.decode()))
                    print_xy(16, 0, "VertSpd:    %s" % (VertSpd.decode()))
                    print_xy(17, 0, "OAT:        %s" % (OAT.decode()))
                    print_xy(18, 0, "TAS:        %s" % (TAS.decode()))
                    ...
                    print_xy(19, 0, "Baro:       %0.2f  Alt:%d""ft" % (converted_Baro, converted_alt))
                    print_xy(20, 0, "DensitAlt:  %s" % (DA.decode()))
                    #print_xy(21, 0, "WndDir:     %s" % (WD.decode()))
                    #print_xy(22, 0, "WndSpd:     %s" % (WS.decode()))
                    print_xy(21, 0, "ChkSum:     0x%s   int:%d " % (Checksum.decode(), intCheckSum))
                    print_xy(22, 0, "CalChkSum:  %s   int:%d " % (calcChecksumHex, calcChecksum))
                    nextByte = ser.read(1)
                    print_xy(23, 0, "endbyte:    %s " % (repr(CRLF[0])))
            else:
                badmessageheaderCount += 1
            ser.flushInput()
            ...
            if len(msg) == 91:
                ...
                if (CRLF[0]) == 13 or (CRLF[0]) == "\r":
                    ...
                    print_xy(7, 30, "Heading Bug: %s" % (HBug.decode()))
                    print_xy(8, 30, "Alt Bug:      %s" % (AltBug.decode()))
                    print_xy(9, 30, "AS Bug:       %s" % (AirBug.decode()))
                    print_xy(10, 30, "VS Bug:       %s" % (VSBug.decode()))
                    print_xy(11, 30, "Course:       %s" % (Course.decode()))
                    print_xy(12, 30, "CDI Type:     %s" % (CDISrcType.decode()))
                    print_xy(13, 30, "CDI Port:     %s" % (CDISourePort.decode()))
                    print_xy(14, 30, "CDI Scale:    %s" % (CDIScale.decode()))
                    print_xy(15, 30, "CDI Deflect   %s" % (CDIDeflection.decode()))
                    print_xy(16, 30, "VertSpd:      %s" % (GS.decode()))
                    print_xy(17, 30, "AP Engaged:   %s" % (APEng.decode()))
                    print_xy(18, 30, "AP Roll Mode: %s" % (APRollMode.decode()))
                    print_xy(19, 30, "AP Pitch Mode:%s" % (APPitch.decode()))
            ...
            if len(msg) == 223:
                ...
                if (CRLF[0]) == 13 or (CRLF[0]) == "\r":
                    ...
                    print_xy(7, 60,  "Oil Pressure: %s" % (OilPress.decode()))
                    print_xy(8, 60, "Oil Temp:     %s" % (OilTemp.decode()))
                    print_xy(9, 60, "RPM:          %s" % (RPM_L.decode()))
                    print_xy(10, 60, "MAP:          %s" % (MAP.decode()))
                    print_xy(11, 60, "Fuel Flow:    %s" % (FF1.decode()))
                    print_xy(12, 60, "Fuel Pressre: %s" % (FP.decode()))
                    print_xy(13, 60, "Fuel Left:    %s" % (FL_L.decode()))
                    print_xy(14, 60, "Fuel Right:   %s" % (FL_R.decode()))
                    print_xy(15, 60, "Fuel Rem      %s" % (Frem.decode()))
                    print_xy(16, 60, "Voltage-1:    %s" % (V1.decode()))
